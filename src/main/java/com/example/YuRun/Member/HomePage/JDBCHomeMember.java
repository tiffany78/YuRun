...
public class JDBCHomeMember implements HomeMemberRepo{
    ...
    public List<RaceUser> getMyRace(String user) {
        ...
        String sql = "select race.id_race, race.title, race.end_date, race.distance, race.description from joinrace join users on users.id_user = joinrace.id_user join race on race.id_race = joinrace.id_race where users.name ilike ? AND end_date > ? LIMIT 3";
    ...
    public List<RaceUser> getUpRace(String user){
        LocalDate currDate = LocalDate.now();
        String sql = "SELECT race.id_race, race.title, race.end_date, race.distance, race.description FROM race LEFT JOIN joinrace ON race.id_race = joinrace.id_race LEFT JOIN users ON joinrace.id_user = users.id_user  WHERE (users.name ILIKE ? OR joinrace.id_user IS NULL) AND end_date > ? LIMIT 2";
        return jdbcTemplate.query(sql, this::mapRowToRaceHome, user, currDate);
    }
}
